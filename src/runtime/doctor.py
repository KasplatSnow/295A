    ...
    def _resolve_models(cls, config: Dict[str, Any], report: DoctorReport):
        ...
        for config_key, path_field, required in path_entries:
            ...
            if section_key == "anomalyclip":
                ...
                # the expected fallback.  Use a descriptive message.
                fix_hint = (
                    "AnomalyCLIP disabled (no deterministic checkpoint "
                    "source configured). Using motion-energy stub. "
                    f"Set {config_key}.hf_repo_id + hf_filename to enable."
                )
            else:
                fix_hint = (
